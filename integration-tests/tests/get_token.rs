...
async fn aborts_and_sends_when_connection_is_never_established() {
    ...
    // Post a valid /get_token request with a short timeout.
...
async fn connects_via_participant_lookup_when_already_present() {
    ...
    // Post a valid /get_token request with delay parameters.
    let mut request = get_token_request(&hs, &user);
    request["delay_id"] = json!("syd_integration_1");
    request["delay_timeout"] = json!(8000);
    let (status, body) = post_get_token(&svc, request.to_string()).await;
...
async fn restarts_the_delayed_event_repeatedly() {
    ...
    // Post a valid /get_token request with a short timeout, so the periodic
...
async fn job_replacement_only_affects_latest_job() {
    ...
    // Post a first /get_token request with delay parameters.
    let mut first_request = get_token_request(&hs, &user);
    first_request["delay_id"] = json!("syd_first");
    first_request["delay_timeout"] = json!(8000);
    let (status, body) = post_get_token(&svc, first_request.to_string()).await;
    ...
    // Post a second /get_token request for the same room and member, but
    // with a different delay_id.
    let mut second_request = get_token_request(&hs, &user);
    second_request["delay_id"] = json!("syd_second");
    second_request["delay_timeout"] = json!(8000);
    let (status, body) = post_get_token(&svc, second_request.to_string()).await;
...
async fn independent_jobs_do_not_cross_talk() {
    ...
    // Post a /get_token request for Alice, in one room.
    let mut alice_request = get_token_request(&hs, &alice);
    alice_request["room_id"] = json!("!room-a:example.com");
    alice_request["delay_id"] = json!("syd_alice");
    alice_request["delay_timeout"] = json!(8000);
    let (status, body) = post_get_token(&svc, alice_request.to_string()).await;

    // The request should succeed.
    assert_eq!(status, 200, "body: {body}");

    // Post a /get_token request for Bob, in a different room.
    let mut bob_request = get_token_request(&hs, &bob);
    bob_request["room_id"] = json!("!room-b:example.com");
    bob_request["delay_id"] = json!("syd_bob");
    bob_request["delay_timeout"] = json!(8000);
    let (status, body) = post_get_token(&svc, bob_request.to_string()).await;
